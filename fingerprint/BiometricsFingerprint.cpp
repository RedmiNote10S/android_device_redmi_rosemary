...
fingerprint_device_t* BiometricsFingerprint::openHal() {
  ...
  std::vector<std::string> possible_fp_modules =
      android::base::Split(android::base::GetProperty("ro.hardware.fp.vendor", "goodix,fpc"), ",");
  ALOGD("Opening fingerprint hal library...");

  for (std::string modname : possible_fp_modules) {
    hw_device_t* device = nullptr;
    const hw_module_t* hw_mdl = nullptr;

    ALOGD("Trying to load fingerprint type %s", modname.c_str());

    if (0 !=
        (err = hw_get_module_by_class(FINGERPRINT_HARDWARE_MODULE_ID, modname.c_str(), &hw_mdl))) {
      ALOGE("Can't open fingerprint HW Module, error: %d", err);
      continue;
    }

    if (hw_mdl == nullptr) {
      ALOGE("Invalid fp module!");
      continue;
    }
  ...
  switch (msg->type) {
    ...
    case FINGERPRINT_TEMPLATE_ENROLLING:
      ...
      if (!thisPtr->mClientCallback
               ->onEnrollResult(devId, msg->data.enroll.finger.fid, msg->data.enroll.finger.gid,
